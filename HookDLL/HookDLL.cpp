/*
http://citforum.ru/programming/digest/smallhook.shtml
*/
...
/******************************************************************/
//TODO: разобраться с char и lpstr https://habrahabr.ru/post/164193/
#include <windows.h>
#include <tchar.h>
#pragma argsused
//---------------------------------------------------------------------------
	...
	void* lpReserved)
{
	if (reason == DLL_PROCESS_ATTACH)	// Проецируем на адр. простр.
	{
		...
		if (RegOpenKey(HKEY_USERS, L".Default\\keyboard layout\\toggle",
			&pOpenKey) == ERROR_SUCCESS)
		{
			RegQueryValue(pOpenKey, L"", cResult, &lSize);
		...
		{
			MessageBox(0, L"Не могу получить данные о способе"
				L"переключения раскладки клавиатуры",
				L"Внимание!", MB_ICONERROR);
		}
		//-------------
		if (!SystemParametersInfo(SPI_GETSCREENSAVEACTIVE, 0,
			&bSCRSAVEACTIVE, 0))
			MessageBox(0, L"Не могу получить данные об установленном"
				"хранителе экрана",
				L"Внимание!", MB_ICONERROR);
	}
	return 1;
